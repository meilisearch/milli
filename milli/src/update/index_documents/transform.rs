...
use crate::{
    ExternalDocumentsIds, FieldId, FieldsDistribution, FieldsIdsMap, Index, MergeFn, BEU32,
};
...
fn compute_primary_key_pair(
    ...
    match primary_key {
        Some(primary_key) => {
            let id = fields_ids_map
                .insert(primary_key)
                .ok_or(anyhow!("Maximum number of fields exceeded"))?;
